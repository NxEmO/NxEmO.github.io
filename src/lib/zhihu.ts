...
function requiredString(record: Record<string, unknown>, keys: string[], label: string, index: number) {
...
function normalizeDate(record: Record<string, unknown>, index: number) {
...
function deduplicate(items: ArticleSummary[]) {
...
function newestFirst(items: ArticleSummary[]) {
...
export function normalizeZhihuArticles(input: unknown): ArticleSummary[] {
  const records = Array.isArray(input)
    ? input
    : input && typeof input === "object" && Array.isArray((input as { articles?: unknown }).articles)
      ? (input as { articles: unknown[] }).articles
      : null;
  ...
  const articles = records.map((value, index) => {
    ...
    return {
      id: requiredString(record, ["id", "articleId"], "an id", index) || externalUrl,
...
export function mergeArticleSummaries(local: ArticleSummary[], zhihu: ArticleSummary[]) {
